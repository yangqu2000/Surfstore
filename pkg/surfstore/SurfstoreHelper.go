...
func WriteMetaFile(fileMetas map[string]*FileMetaData, baseDir string) error {
	...
	for filename, fileMetaData := range fileMetas {
		...
		for hashIndex, hashValue := range fileMetaData.BlockHashList {
...
func LoadMetaFromMetaFile(baseDir string) (fileMetaMap map[string]*FileMetaData, e error) {
	...
	distictFileName, err := db.Query(getDistinctFileName)
	if err != nil {
		log.Fatal("Error When Query Distinct File Name")
	}

	var fileName string
	for distictFileName.Next() {

		distictFileName.Scan(&fileName)
		tuples, err := db.Query(getTuplesByFileName, fileName)
		if err != nil {
			log.Fatal("Error When Query Tuples By File Name")
		}
		...
		for tuples.Next() {
			tuples.Scan(&version, &hashIndex, &hashValue)
